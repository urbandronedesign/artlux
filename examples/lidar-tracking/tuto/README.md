...
4. **Start the feed** (projects 1–2 only), from the repo root:
   ...
   **Tracking** context: a **green** status dot (emerald once traffic arrives, amber if OSC is on but
...
read even though it is not baked into the shipped projects — and you can build and tune the whole thing
...
<!-- TODO screenshot: Tracking context ▸ Trigger Zones dock tab. Left: the SOL flat map (bottom-left origin), one drawn amber zone lit "occupied" with "· 2" headcount, raw cyan blobs; right: the zone list with eye toggles + People needed + Override dwell. -->
