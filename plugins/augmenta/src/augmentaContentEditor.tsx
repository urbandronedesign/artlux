...
// The per-surface inspector fragment for AUGMENTA content, rendered by the host ContentEditor via the
// content-source provider's `editor` hook. Configures the marker viz + orientation calibration (the
// same flip/rotate/showIds/trail fields the TRACKING + MEDIAPIPE content reuse) — so an Augmenta
// surface tunes exactly like a LiDAR or camera-pose one. Augmenta is a single field, so there's no
// source picker; the box's real-world field size drives the source aspect automatically.
...
export const AugmentaContentEditor: React.FC<{ content: SurfaceContent; onChange: (patch: Partial<SurfaceContent>) => void }> = ({ content: c, onChange }) => (
  <div className="space-y-2 pt-1">
    <Slider label="Marker size" value={c.blobSize ?? 0.04} min={0.01} max={0.2} step={0.005}
    ...
    {c.trail !== false && (
      <Slider label="Trail (s)" value={c.trailSeconds ?? 1.2} min={0} max={3} step={0.1}
        format={(v) => `${v.toFixed(1)}s`} onChange={(v) => onChange({ trailSeconds: v })} />
    )}
    <Toggle label="Show IDs" checked={c.showIds ?? false} onChange={(v) => onChange({ showIds: v })} />
    <Toggle label="Calibration overlay" checked={c.calibration ?? false} onChange={(v) => onChange({ calibration: v })} />
    <div className="flex items-center justify-between gap-2 text-xs">
      <label className="text-fg-2 w-20 truncate" title="Rotate the tracking field">Rotate</label>
