// Stage-side drawable for MEDIAPIPE surface content: renders the smoothed people markers (+ optional
// background video + skeleton/#id overlay) on the GPU into a per-surface WebGL2 canvas and returns it
// as the surface's drawable (composited by the stage, sampled by the mappers). Mirrors the LiDAR
// trackingDrawable; the projector renders the same content into poseProjector's source FBO. A minimal
// 2D fallback is used only when WebGL2 is unavailable.
...
// Render pose content keyed by an arbitrary consumer key (surface id, or `layer:<id>` for a timeline
// content clip). The renderers read only `content` (+ the key for the canvas map).
...
function renderGL(g: GLSurf, surface: Surface, w: number, h: number, now: number): HTMLCanvasElement {
  ...
  if (c.bgLayerId) {
    ...
    if (bg && posePass.uploadTexture(gl, g.bgTex, bg as TexImageSource)) posePass.drawTex(gl, g.bgTex, 1, false, false);
  }
  if (c.trail !== false) {
    const trail = poseRenderer.trails(surface, now, c.trailSeconds ?? 1.2);
    if (trail.length) posePass.drawSolid(gl, trail, false);
  }
  posePass.drawMarkers(gl, w, h, poseRenderer.instances(surface, now), false);
  const ov = poseRenderer.overlayCanvas(surface, w, h);
  if (ov && posePass.uploadTexture(gl, g.overlayTex, ov)) posePass.drawTex(gl, g.overlayTex, 1, true, false);
  return g.el;
}

// Minimal 2D fallback (no WebGL2): solid disc per person + bg + overlay. Cheap, rarely used.
function renderCPU(surface: Surface, w: number, h: number, now: number): HTMLCanvasElement {
  ...
  const r = Math.max(2, (c.blobSize ?? 0.05) * h);
  for (const b of poseRenderer.instances(surface, now)) {
    ctx.globalAlpha = b.a;
