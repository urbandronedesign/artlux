...
| **mp4** | ✅ shipped | GPU WebCodecs H.264/H.265 `.mp4` decode, the second **`VideoCodec`** contribution. Renderer-only; **on by default** since the engine-decoupling work (`mp4WebCodecs` absent ⇒ true) — a track WebCodecs cannot configure declines at probe time and falls back to a `<video>`. See the Video-codecs section below + [CODECS.md](CODECS.md). |
