...
| `automationTargets` | `AutomationTargetProvider` | **Publishes automatable parameters into the core curve engine** — `enumerate()` the targets a plugin owns, `write()`/`writeFade()` the two override layers, `release()`/`releaseFade()` to hand a path back, `get()` (authored) / `getLive()` (what is actually sounding). This is how an audio lane on the timeline is *the same object* as any other lane. A target may also declare `display { unit, min, max }` — a linear map from storage onto the unit the operator reads (a Pan lane showing `270 deg` while storing `0.50`). It is read by the lane UI only: `min`/`max` are what `compileAutomation` **clamps** to and what reaches `write()`, so publishing the display range there would let a curve be drawn past the parameter's real limit. ⚠ Undocumented here until 2026-07-14 — the table listed 8 of the 9 registries. | audio |
...
The dockable workspace — **the default shell since v0.25.0** ([WORKSPACE.md](WORKSPACE.md), [plans/dockable-workspace.md](../plans/dockable-workspace.md)) — lets an operator rearrange a context into split and tabbed groups. **`WorkspaceContext` is untouched, and no contribution shape changed.** The arrangement is *compiled* from the flat manifest a context already declares — `browser[]` / `dock[]` / `inspector[]` / `viewport` — so a plugin keeps declaring what it declares today and lands where it always did.
...
- **Two renderers can mount your panel**, since the pre-docking fixed shell still ships as a fallback. Declare behaviour (`appliesTo`, `bare`, `grow`, `HeaderActions`) in the *registration* and never infer it from the container you think you are in — `appliesTo` was silently dead under docking for one release because only the hand-built column applied it.
