...
| **4** | **The JUCE / libspatialaudio licensing call is made.** JUCE is dual-licensed (a paid tier, or AGPL — which is viral); libspatialaudio is **LGPL-2.1** (dynamic-link, or comply). **The obligations are recorded in [`NOTICE`](../../NOTICE) and README ▸ Licensing — read those, and do not trust any dollar figure quoted from memory in this repo; they change between JUCE versions.** | **You** | ⏳ (still open — recorded, not decided) | It does **not** block the merge. It blocks the **first `v*` tag** (a tag publishes a Release, and the addon ships JUCE + libspatialaudio linked in). **Decide it before it ambushes a release.** |
