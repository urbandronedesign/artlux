...
**Infinite & looping:** the playhead runs unbounded — build sequences as long as you like. Looping is
off by default; toggle **Loop** (**Shift+L**) to repeat the **in/out** region.
...
- Connect them with **transitions** that fire **manually** (buttons on the state lane) or
  automatically **after a delay**, **at a time**, **on a marker**, or **when a clip ends**.
