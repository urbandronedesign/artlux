...
const ScenePanel3DBase: React.FC<ScenePanel3DProps> = ({
  ...
  return (
    <div
      className="absolute top-2 right-2 z-10 w-60 bg-surface-1/95 backdrop-blur-sm border border-line-1 rounded-[var(--r-md)] text-xs flex flex-col max-h-[calc(100%-1rem)]"
      onPointerDown={(e) => e.stopPropagation()}
    >
      <div className="flex items-center justify-between px-3 h-9 border-b border-line-1">
        <button onClick={() => setCollapsed(c => !c)} title={collapsed ? 'Expand' : 'Collapse'} className="flex items-center gap-1.5 text-[11px] font-bold text-fg-1 hover:text-accent">
      ...
      {!collapsed && (
        <div className="flex flex-col min-h-0 overflow-y-auto">
          ...
          <div className="p-1 space-y-0.5 max-h-40 overflow-y-auto">
            ...
            {models.map(m => (
              <div key={m.id} className={rowCls(selectedModelId === m.id)} onClick={() => onSelectModel(m.id)}>
          ...
          <div className="p-1 space-y-0.5 max-h-40 overflow-y-auto">
            ...
            {fixtures.map(f => (
              <div key={f.id} className={rowCls(!selectedModelId && selectedFixtureId === f.id)} onClick={() => onSelectFixture(f.id)}>
              ...
              {selModel.kind !== 'plane' && (
                <>
                  <div className="flex gap-1.5">
                    {[1, 10, 100, 1000].map(v => (
                      <button key={v} onClick={() => onUpdateModel(selModel.id, { scaleXYZ: [v, v, v] })} className="flex-1 px-1 py-0.5 rounded-[var(--r-sm)] bg-surface-2 border border-line-1 text-fg-2 hover:text-fg-1 num text-[10px]">×{v}</button>
                    ))}
                  </div>
                  <div className="flex items-center gap-1.5 text-[11px]">
