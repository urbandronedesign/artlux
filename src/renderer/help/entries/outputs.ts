...
export const outputsHelp: HelpEntry[] = [
  ...
  },
  {
    ...
    body: 'These come BEFORE any software warp: they help you aim, zoom, roll and focus the real machines so their images land where you want and overlap by the right amount. By default every pattern is drawn in the projector\'s raw raster, unwarped, because you are adjusting where the light goes — an aid that moved with the corner-pin would hide the error you are hunting. Once the machines are hung, "Follow the warp" flips that (see its own entry). Each output is tinted its own colour (the first three are red/green/blue, so an overlap reads as their mix, and the swatch in each row tells you which light is whose). Grid gives lettered columns and numbered rows so two people can name the same square. Blend is the overlap tool: your soft edge drawn as a hatched band with its inner boundary bright and a ladder across it — match the neighbour\'s ladder and you have matched zoom, aim and roll at once. Focus, Greys, Bars and 1:1 cover sharpness, brightness/gamma match, colour match and native resolution. Dim controls how far the show underneath is darkened. Nothing here is saved with the project.',
    group: 'Outputs',
    keywords: ['align', 'overlap', 'blend', 'test pattern', 'grid', 'focus', 'projector setup', 'rigging', 'multiscreen', 'edge blend'],
  },
  {
    id: 'outputs.align-aids-warp',
    title: 'Follow the warp',
    short: 'Bend the aid pattern with the picture instead of drawing it in the raw raster.',
    body: 'A rig setup has two halves and they want opposite things. While you are on the ladder the aid must stay in the projector\'s raw raster — you are moving light, and a grid that followed the software warp would hide the very error you are chasing. Once the machine is hung and you are shaping the picture with the corner-pin or the Bézier mesh, that same unwarped grid answers a question nobody is asking: you now need to see where the WARPED picture\'s edge falls against its neighbour\'s. Turn this on and the pattern goes through exactly the geometry the content takes, so grid squares land on the picture and two outputs\' grids can be matched across an overlap after warping. Nothing else is applied to it — no soft edge, no gamma, no brightness — because an aid you compare against a neighbour must not be feathered to nothing in the overlap where you compare them. 1:1, White and Black stay in the raw raster always; the checker measures device pixels, so bending it would destroy what it is for. On an output playing a baked calibration file the pattern also stays in the raster, because a baked map has no inverse to bend an aid through. Dim keeps working — the darkening travels into the picture with the pattern, so the aid stays bright on top of a dimmed show — but note that a warped pattern is now IN the picture, so it also reaches that output\'s NDI send and any capture of it, which the unwarped aid does not. Costs one extra blended pass per output while an aid is up, and nothing at all when none is.',
    group: 'Outputs',
    keywords: ['warp aids', 'follow warp', 'corner pin', 'four point', '4 point', 'distort', 'align', 'test pattern', 'grid', 'multiscreen'],
  },
];
