...
| **C1** | **Checkpoint — user + rig soak (dev *and* packaged)** | ⏳ **awaiting the user** | | **Phase 1 is code-complete.** Done here on 2026-07-27 against the operator's real project (`Documents/projetled/artlux-project.artlux`, 2 surfaces / 2 fixtures / 6 scenes / 23 assets / enabled 6-state machine): **4345 ArtDmx packets sniffed on universes 0–3**, 60 Hz native / 61 fps renderer, full nine-context tour with output never dropping, headless verified separately. **Still needs the user:** the packaged `ArtLux.exe` with no CDP port, real fixtures on the wire, and a broadcast/projector run. Also fixed en route (pre-existing, unrelated): 8 asset paths in that project pointed into a deleted temp dir — repointed to the project's own `assets/`, backup at `artlux-project.artlux.bak-before-path-fix`. |
