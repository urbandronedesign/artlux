...
**The Phase-0 UI thread is finished, and it is worth knowing where it landed** (WP-0.M). Idle React cost
**177–224 → 0.0 ms/s**, and the frame-timing tail with it: **p99 54 → 21 ms, long frames 18 → 0**. Under
heavy UI load the p99 is **unchanged** (155 → 168) — which is the finding, not a failure: that hitching
was never React commit cost, it is the context switch plus forced layout. Do not spend another session
memoizing things hoping to move it.

What is left, measured:

- **A clip drag: ~330–380 ms/s**, after WP-0.6 took the ruler and toolbar out of it. The remainder is
  **Timeline's own body, ~10 ms per render** — its element tree and hooks — which no memoization
  reaches. The real fix is to make the drag render-free (move the clip by style, commit on release,
  the way the playhead and the automation readouts already work). That is a design change, not a tweak.
- **A context switch: ~70–105 ms, once**, and several contexts cost nothing. Paid for a real event —
  a person changed the whole workbench — so this is a judgement call, not an obvious yes.

⚠ **Harness rules, each of which produced a confident lie before being caught.** The UI profiler is
opt-in via `localStorage['artlux.uiPerf']` — **`?uiperf=1` does not survive the app's startup
navigation**, and a profiler that is off reports `0`, which reads exactly like "free"; assert it is
non-zero before believing any measurement. A reload can restore a full-page route (Preferences) that
unmounts the whole shell, so nothing is profiled. A clip's rect is **not** a hit target: the drawer lays
lanes out below a 908 px window, so check `elementFromPoint` actually hits the clip. And a page-context
`import()` of `uiPerfMonitor` gives a **second instance** that reports zero forever — read
`artlux_ui_commit_ms` from `/metrics` (pushed by App at 1 Hz, so wait ~1.2 s after an event) or the
Performance panel's own rows.
...
| 0.4 | Stabilize App's handler identities, then memoize the viewports | ☑ done *(Stage only — see why)* | `eced6ea` | **The mechanism works:** `useStableHandlers` (the `EditorStore` Proxy trick, extracted so App can use it for callback *props*) plus a memoized toolbar make every Stage prop referentially stable, and `viewport:stage2d` commit time during a context tour goes **1.2 ms → 0.0 ms** — the memo genuinely bails out rather than paying for a compare that always fails. **But 1.2 ms was never the problem, and the measurement is worth more than the change.** The same run shows **`viewport:timeline` burning 224 ms/s across 10 commits** (~22 ms each) during that tour — on the panel open in **eight of nine contexts** — against scenes 13.7 ms, outputs 2.5 ms, stage 1.2 ms. **That is where the p99 54 → 155 ms hitching measured in 3.M comes from.** The other viewports were deliberately **not** memoized: at 2.5–13.7 ms they do not justify the prop-stabilisation churn, and doing them would have buried the real finding. **Next UI-cost work should go straight to `TimelinePanel`**; the technique is now sitting in `hooks/useStableHandlers.ts`. ⚠ Measuring commit *counts* would have missed all of this — the Profiler reports its whole subtree, and App rebuilds the wrapper `<div>` every render, so the count stayed 1 → 1 while the **time** went 1.2 → 0. |
| 0.5 | The timeline stops re-rendering itself on a clock *(added — WP-0.4's finding, chased down)* | ☑ done | `34904d9` | **The 224 ms/s was not the context tour. It was a `setInterval(…, 100)` inside `Timeline.tsx`.** It sampled the playhead AND the show clock into React state so the automation lanes could print a live value in their gutter — ten full renders of the panel per second (toolbar, ruler, every track header, every clip, every lane), **for ever, in eight of the nine contexts, whether or not the transport was moving and whether or not a single automation lane existed**. WP-0.4 read "10 commits in a 1 s window during a tour" as tour cost; 10 Hz was the interval. So no amount of prop-stabilisation at the App boundary would have touched it — the render originated *inside*. The lanes now take the **name** of their clock (`clock: 'playhead' \| 'show'`) and read it themselves, writing the readout straight to the DOM through `engine.subscribe` — the same render-free discipline the 60 Hz playhead and timecode have always used. The surviving 1 Hz target re-enumeration (how a lane notices its target was deleted) now compares a signature and returns the **same object** when nothing changed, so it costs nothing in the overwhelming majority of seconds where nothing has. **A/B measured on the operator's real project, app idle, via the app's own `artlux_ui_commit_ms`: mean 177 → 0.0 ms/s, peak 207 → 0.0, every sample zero.** Functionally verified against a purpose-built project with a ramp on **both** clocks (a bound scene's own lane on the playhead, the global base lane on the show clock): both readouts took **20 distinct values in 20 samples** — smoother than the 100 ms interval they replaced — at **0.0 ms/s** of commits. **Two side fixes:** the "add a keyframe here" button read the stale 100 ms sample for both the key's `t` and its `v`, so a key landed up to three frames behind the click; it reads the clock at click time now. Guard `the timeline never puts a clock in React state` (4 break tests, all fired). ⚠ **Harness lessons, both of which produced a confident lie before being caught:** `?uiperf=1` does **not** survive the app's startup navigation — use `localStorage['artlux.uiPerf']`, and *assert the profiler is non-zero before measuring*, because a profiler that is off reports 0 and 0 is indistinguishable from "free"; and a reload can restore a full-page route (Preferences), which unmounts the whole workspace shell so **nothing** is profiled. |
| 0.6 | The clip drag stops rebuilding the ruler *(added — the interaction cost WP-0.5 did not touch)* | ☑ done | `f55fea6` | WP-0.5 removed the timeline's *idle* cost. This is the *interaction* one: a clip drag setStates a draft per pointer move, measured at **idle 0.0 → 277–493 ms/s while dragging**. Attribution with temporary in-panel profilers: **`tl:ruler` 170 ms of the panel's 341**, toolbar 61, takes 17, **lanes 9.5**, auto 8. **The instinct was to memoize the lanes; the measurement said the ruler** — it renders one tick element across the full width, and the timeline is unbounded, so that width grows as the show runs (**measured live at 32,957 px, ~800 ticks**). Ruler + toolbar are `React.memo` now, with their callbacks arriving as one `useStableHandlers` bag, `overrun` memoized, and `markers` no longer `timeline.markers ?? []` (a fresh array per render of the common case — the one thing that would have made this inert). **Proven to bail rather than compare-and-fail:** re-probed mid-drag, `tl:ruler` reports **0.1 ms across 35 commits**, toolbar 0.1 ms — the WP-0.4 pattern again (count stays, time collapses). ⚠ **Honest about the end-to-end number:** one A/B gave **493 → 378 ms/s peak**, another sat inside run-to-run noise. The finding explains it — the ruler's cost scales with how far the timeline has grown, so this is small on a freshly opened project and large in the long session a venue runs. Functionally verified live (a memo + handler Proxy can leave a control that looks right and does nothing): zoom redraws the ruler 32,957 → 49,435 px, Snap toggles, Add Track adds, Add Marker puts a flag on the ruler, no `stableHandlers` warning. Guard `the timeline ruler and toolbar are memoized, and handed props that hold still` (4 break tests). **What is left in that panel is ~10 ms per render of Timeline's own body** — its element tree and hooks — which no memoization reaches; making the drag render-free (move the clip by style, commit on release) is the next idea, and a design change rather than a tweak. |
| **0.M** | **Did the Phase-0 UI work move the number 3.M measured?** *(the closing measurement for this thread)* | ☑ done | `_pending_` | Re-ran WP-3.M's stress on the same project, **profiler off** (it times every render in its subtree; leaving it on would measure the instrument). **The steady state is fixed outright — the load case is untouched, and that is the useful half of the answer.**<br>• **Idle: frame p99 54 → 21 ms, long frames 18 → 0 / 240.** 21 ms is about one frame at 60 Hz, i.e. there is no longer a tail. That is WP-0.5 showing up in the *frame-timing* metric rather than only in the React one — the two are the same 177 ms/s seen from either end.<br>• **Under UI load: p99 155 → 168 ms, long frames 40 → 47** — unchanged, arguably slightly worse. So **the load-case hitching was never React commit cost.** It is the context switch itself (~70–105 ms, measured separately) plus the harness's own 40 forced synchronous layouts per switch. No amount of memoization reaches either.<br>• Output through the load: **61.9 → 54.7 Hz, worst second 50 Hz**, 141 packets/s throughout — the wire dips in rate but never stops, exactly as 3.M found. `artlux_ui_blocked_ms` did register this one (372 ms/s), unlike the 900 ms freeze it under-reported.<br>**Therefore 3.M's conclusion still stands, for a better reason than before:** a worker would buy smoother content under heavy UI load, and now that the idle cost is gone, that load — not React — is demonstrably all that is left to buy. |
...
| **C2** | **Checkpoint — media soak (user)** | ⏳ **awaiting the user** | | **Phase 2 is code-complete.** Done here: MP4/WebCodecs, images, DMX-in and HAP all verified **on the wire** against real media from the operator's own project. **Not exercised — no source on this machine:** **Spout** (no sender), **NDI** (no source), **camera pixels** (no working camera; Chromium enumerates 5 video inputs, none deliver frames — confirmed by an A/B against HEAD, equally black). Those three are what C2 is for. ⚠ **Not performed — the user said "go" and Phase 3 started without it.** Three unverified layers are now stacked (C1's rig, C2's three sources, then the worker move), and Phase 3 relocates the entire output path. Recorded so it cannot quietly disappear. |
| 3.1 | Output MessagePort plumbing (main ↔ preload ↔ renderer) | ☑ done | `dc01ad9` | Main opens a `MessageChannelMain` per load and posts one end via the **projector-port recipe** (preload relays with `window.postMessage`; a port can't cross the contextBridge). Payload is **identical to `IPC.FRAME`** (`encodeFrame` output) so the two engine modes can't drift; a missing port **falls back to IPC**. Its own readiness signal, not the projector queue — the port arrives in *every* mode including headless, where nothing posts `projector-ready`, so sharing would have parked it forever in the run that needs it most. **THE FINDING:** `postMessage(buf, [buf])` — the standard zero-copy idiom, legal on a DOM MessagePort — **does not work across Electron renderer→main**. The message *arrives* (handler fires, on time, every frame) but **`e.data` is `null`**: no error, no warning, nothing in either console, just empty messages and no output while the transport reports ready, the port reports alive and the renderer sits at 61 fps. Only logging the payload shape at the far end revealed it. Frames are now **copied** (539 bytes at a 44 Hz cap — a rounding error) and the mistake is **guarded**, being the most plausible "optimisation" someone would make there. **Verified on the wire:** 834 ArtDmx packets, 274 distinct sums, native 63 Hz / 1 universe, renderer 61 fps; headless re-checked. |
| 3.2 | Engine hosted in the worker + main-thread proxy + mode flag | ⛔ **BLOCKED — plan assumption does not hold** | | **The platform is fine; the import graph is not.** Probed in this Electron: a Worker has `navigator.gpu` with an **acquirable device**, `OffscreenCanvas`, `VideoFrame`, `VideoDecoder`, WebGL2, and takes a transferred `MessagePort`. So nothing about the runtime blocks this. What blocks it is that `frameEngine` reaches DOM-bound media through its imports: **`contentSource` creates `<video>` elements** for any video the codecs decline (and calls `window.artlux.configureInput`, which does not exist in a worker), and **`services/timeline` keeps a per-layer `<video>` pool** *and* is the show engine (FSM, transport, audio-driver ordering). Phase 2 moved images → `ImageBitmap`, camera → `VideoFrame`, byte-sources → `OffscreenCanvas`, HAP → `OffscreenCanvas`+WebGL2 — **but not the `<video>` element**, which is still the fallback producer for both surfaces and timeline layers. The plan's own Wave-2 gate said this step is "only viable after media production is `VideoFrame`-based"; that is not yet true. **Three shapes forward, costed in the summary — this needs a decision, not an improvisation.** |
| **3.M** | **Measure what the worker would actually buy** *(added — the user's call at the WP-3.2 fork)* | ☑ done | `967648d` | **The wire is already immune to a frozen renderer.** During a deliberate **900 ms main-thread freeze** on the operator's real project: **172 packets over 967 ms (178/s)** vs 226/s idle, worst inter-packet gap **45 ms** — that is the native Rust pacer re-sending the last frame, so the wire stays alive and only the *picture* stops advancing. **Ordinary UI stress does not starve output either:** three rounds of full context tours plus forced reflow gave **0 gaps over 100 ms**, worst gap 68 ms, output 49–60 Hz. What *does* degrade is frame timing — render p99 **54 → 155 ms**, long frames **18 → 40**: hitchier content, not a stopped show. **A worker would buy smoother content under heavy UI load, not show survival.** ⚠ **It also exposed a flaw in WP-0.1's own instrument:** `artlux_ui_blocked_ms` reported **101 ms for a 900 ms freeze**, and only in the sample **1.4 s later** — it under-reports and lags, so `0 ms/s` means *no evidence of blocking*, **not** *no blocking*. Do not read that gauge as proof of a quiet main thread. |
| 3.3 | Plugin IPC relays into the worker | ⏸ **deferred with 3.2** | | |
| 3.4 | Projector frame pump — **verbatim** lift-and-shift + parity checklist | ⏸ **deferred with 3.2** (it was only forced BY the worker move) | | |
