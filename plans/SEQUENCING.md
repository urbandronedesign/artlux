...
*Update the Status cell to `☐ in progress (branch cut)` → `☑ merged <date>` as each wave lands.*

> **⚠ RECONCILED AGAINST THE TREE, 2026-07-29.** This table had drifted in **both** directions and was
> repaired by grepping for each plan's actual deliverables rather than by memory. Six plans that had
> shipped were missing or marked pending (help-merge, dockable-workspace, engine-decoupling,
> fixture-kinds/lighting-keyframes, multi-projector-blend, and the `timeline-undo` half of Wave 4);
> nothing was marked done that had not shipped.
>
> **The trap this hit, recorded so the next reconciliation avoids it:** *a file existing does not mean
> its plan shipped.* `useHistory.ts` predates `timeline-undo` by three weeks (it came with the original
> Electron port), and `ErrorBoundary.tsx` came from an **a11y** commit, not from
> `renderer-error-containment`. Both were briefly read as proof that Wave 4 was complete. Check for the
