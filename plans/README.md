...
| [MIDI controller support](midi-control.md) | (net-new) | **Plugin** | 🟡 Med | not started (Draft). ✅ **unblocked 2026-07-29** — ships its usage docs with it |
...
| [Renderer error containment](renderer-error-containment.md) | #10 Ship It | **Core** | 🟠 Med | Draft — **NOT started** (re-confirmed 2026-07-29: no `faultReporter.ts`, no fault channel, no pre-first-heartbeat arming). **The watchdog is blind to a white screen.** A first-render throw means the heartbeat never fires, so the watchdog **never arms** and an unattended install sits dead until someone drives to the venue. ⚠ `ErrorBoundary.tsx` exists but is an **a11y**-era panel boundary, not this plan — §1 says wrapping panels would have caught neither shipped white-screen |
...
[MIDI controller support](midi-control.md) is the one still-active net-new plan (**unblocked 2026-07-29**
— the documentation gate is open). The two that have shipped
