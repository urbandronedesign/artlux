...
| [**Offline bake — surface → MP4 + AAC**](offline-bake.md) | (net-new) | **Plugin** (`plugins/bake`) + 4 core seams | 🟡 Med | 🟩 **BUILT, ready to merge** on branch `offline-bake`. Render a surface's content **non-realtime** (as slow as it needs) to an MP4 the show then plays back as a cheap decode. **Phase 0 measured 2026-09-11:** H.264 hw encode 119 fps @1080p60 / 35.8 @4K on the *Intel* box — the encoder is not the bottleneck; **HEVC encode unsupported**, **AAC stereo-only**. Two review passes killed two designs: swapping `Surface.content` is reverted silently by the FSM's scene recall on every state entry (so bakes are project-scope, like `assets`), and mp4's `thumbnail()` is **not** frame-exact (so `frameExact?()` joins the SDK). Bakes are **upstream of all projector warp/blend/calibration**, so they survive re-calibration. |
...
| [Trigger zones — people, and per-zone rules](trigger-zone-rules.md) | (bug fix + net-new) | **Core + lidar-tracking plugin** | 🟡 Med | ⚠ **BUILT + PUSHED, branch `trigger-zone-rules` 2026-09-07 — not merged** (`a9a252d`, `8d5fc12`). Answers a venue report — *"the lidar tracks 2 blobs per person so I had to put 4 instead of 2"* — and the follow-up that reframed it: on the **wall** a blob is a **hand**, so what a blob *means* is a property of the **surface**, not the project. One project-wide merge flag was silently merging wall hands into one trigger. Four causes in all: per-surface meaning; zones and projectors running **different algorithms** (a bare spatial merge vs the predictive tracker, so the number the operator validates against was not the number the show acts on); the sensor settings **riding the look** and being reassigned on every GO; and no way to see the closest-pair distance that decides the merge radius. Part B then gives **every term of a combination its own rule** — *"Zone 1 occupied for 5 s AND Zone 2 empty"* in one edge — which turned out to be an *extraction*, because `level()` always computed all five kinds as levels and the combination path used a hardcoded subset. Zero migration, byte-identical edge labels. Two bugs found on the way: an arm-and-hold memo key that would have fired a transition **60×/s** with nothing thrown, and live occupancy dots that **had no subscription at all**. ⚠ **The end-to-end harness cases are written and were never run green** — see the doc's *What is NOT proven*. |
