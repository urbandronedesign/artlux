...
| **5.4b** | **Three layout bugs found by the operator in five minutes of real use** *(added)* | ☑ done | `1f7202c`, `f5d35de` | All three were in /the splitter, all invisible to every automated check, and all diagnosed by **asking the running app for computed geometry** rather than reading the JSX.<br>**1. A fixed pane could not shrink.**  reads like a width and in a roomy window is one; in a 1264×541 window the dock kept 293px of a 200px workspace and the difference painted *outside* the box, over the timeline drawer — a permanently black lower half. Panes shrink now, a px basis is capped at 45% of its split, and the split clips. Measured: dock **293 (overflowing) → 90**, stage **0 → 46**.<br>**2.  factors summing under 1.** Flexbox distributes only that fraction of free space and leaves the rest empty — a viewport at  beside a fixed dock filled 43% and left a **black band across the middle** of an ordinary 1440×900 window. The 0.43 came from our own splitter commit.  is now normalized by its split total, so the sum is 1 by construction and no already-saved tree can reproduce it. Measured: stage group **132 → 303**, exactly filling the 559px workspace.<br>**3. A drag pinned every pane.** The splitter writes pixel sizes onto the pane elements to stay off the React path — but **React writes a style property only when its own props change**, and an fr pane computes to the same style before and after a drag, so  stayed on the element for good. Every pane then had grow 0 and new space went to nobody: a strip down the right edge (panes totalling 1808 in an 1872 box) and a band under the dock (720 in 790). The panes assert their flex **imperatively after every render** now; this cannot be declarative, because "the value did not change" is exactly the case needing repair.<br>**The lesson worth keeping:** two window heights (908, 1700) and a synthetic project hid all three. Real use at a real size found them immediately. Guards added for each (7 assertions, all break-tested); two guards had to be **tightened first** because they matched text that survived the break. |
