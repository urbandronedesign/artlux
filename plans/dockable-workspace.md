...
> **Status:** **shipped 2026-07-27** (WP-5.1 → 5.6; dockable by default, both paths ship one release) · **Placement:** **Core** (`renderer/services/dockTree.ts`,
...
| 5.4 | Interaction: tab drag + 5-zone drops, splitters, collapse, close, context menu, Add-Panel, Reset | ☑ done | `ab8ec95` | **Verified live, 14 checks:** drag between groups, edge-drop splits, Add, Close, **survives a restart**, a closed panel is **not** resurrected, Reset restores, exactly one element per persistent viewport, output 61 Hz, **idle React 0.0 ms/s**, no renderer errors. **One real bug found by the test:** a viewport group drew no tab strip, so a panel dropped into it was invisible *and* unreachable — it now gets one as soon as it holds more than the viewport. Guard `docking drags on pointer events, resets by recompiling, and cannot dock a modal panel` (4 break tests; the HTML5 check had to be **widened** — it only matched `draggable=`, so a bare `draggable` attribute, the shortest way to reintroduce what it forbids, walked past it). ⚠ **Two of my own test assertions were wrong before the code was:** dropping into a single-viewport group makes a strip appear, so tab count jumps by two, not one; and Reset has no reason to restore a panel that was only ever added by hand. **Process note:** a debug `git checkout` on an uncommitted file destroyed this WP mid-session and it had to be reapplied — the reapply script asserts every one of its 12 edits, because a silent no-op replace had already cost a wasted break-test cycle earlier the same hour. |
| **5.4b** | **Three layout bugs found by the operator in five minutes of real use** *(added)* | ☑ done | `1f7202c`, `f5d35de` | All three were in `sizeToFlex` and the splitter, all invisible to every automated check, and all diagnosed by **asking the running app for computed geometry** rather than reading the JSX.<br>**1. A fixed pane could not shrink.** `flex: 0 0 280px` reads like a width, and in a roomy window it is one; in a 1264×541 window the dock kept 293px of a 200px workspace and the difference painted *outside* the box, over the timeline drawer — a permanently black lower half. Panes shrink now, a px basis is capped at 45% of its split, and the split clips. Measured: dock **293 (overflowing) → 90**, stage **0 → 46**.<br>**2. `fr` factors summing under 1.** Flexbox distributes only that fraction of the free space and leaves the rest empty — a viewport at `fr: 0.43` beside a fixed dock filled 43% and left a **black band across the middle** of an ordinary 1440×900 window. The 0.43 came from our own splitter commit. `fr` is now normalized by its split total, so the sum is 1 by construction and no already-saved tree can reproduce it. Measured: stage group **132 → 303**, exactly filling the 559px workspace.<br>**3. A drag pinned every pane.** The splitter writes pixel sizes onto the pane elements to stay off the React path — but **React writes a style property only when its own props change**, and an fr pane computes to the same style before and after a drag, so `0 0 1056px` stayed on the element for good. Every pane then had grow 0 and new space went to nobody: a strip down the right edge (panes totalling 1808 in an 1872 box) and a band under the dock (720 in 790). The panes assert their flex **imperatively after every render** now; this cannot be declarative, because "the value did not change" is exactly the case that needs repairing.<br>**The lesson worth keeping:** two window heights (908, 1700) and a synthetic project hid all three. Real use at a real size found them in minutes. Guards added for each (7 assertions, all break-tested); two had to be **tightened first** because they matched text that survived the break. |
| 5.5 | Guards + docs | ☑ done | `53f1833` | WORKSPACE.md gains a full section (the tree, the drag, the persistent layer, the three layout traps) and its out-of-scope note stops saying nothing is built. CLAUDE.md gains the one rule a contributor can break without noticing: docking **positions** the persistent viewports, and the panes assert flex **imperatively**. SDK.md says what plugin authors need: `WorkspaceContext` untouched, no contribution shape changed, late panels still land in their region, modal panels structurally not dockable. **Guard sweep: all 10 docking assertions across 3 suites re-broken and seen to fire.** 57 checks. |
| 5.6 | Flip the default on; keep both paths one release | ☑ done | `5d3e644` | **The flip is a RENAME, not a value change.** It shipped as `docking?: boolean` defaulting to false, so every install that saved a layout while it was opt-in has `docking: false` in its prefs — flipping the default would have reached nobody who had already used the app, which is the failure mode `layoutRev` exists for. The key is now `dockingOff?: boolean`: absence means the new default, so the flip lands everywhere and only a deliberate opt-out persists. **Two ways back**, because a venue needs one that is not a build: *Preferences › Appearance › Dockable workspace* for the operator, and `localStorage['artlux.docking'] = '0'` per machine. **Verified on a fresh install with every flag cleared:** 4 groups with draggable tabs, all five persistent viewports mounted exactly once, drawer still outside the tree; then off → the hand-built shell returns with its column resizers, viewports still mounted once; then on → back to the tree. Output held 61 Hz throughout, no renderer errors. ⚠ The check found nothing wrong with the code and one thing wrong with itself: its first version clicked the first checkbox in the settings card (the splash setting) and reported success while changing nothing. |
