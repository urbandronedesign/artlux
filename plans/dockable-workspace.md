...
|---|---|---|---|---|
| 5.1 | `services/dockTree.ts` — types, `sanitizeDockTree`, `normalize`, the pure ops, `defaultTreeOf`, `mergePluginPanels`. **No UI.** Import-free, so it is verifiable by a throwaway `tsc`-checked script (the repo's documented pattern for pure logic). | ☐ | | Accept: round-trip script covering every op + idempotence + the caps; guard that sanitize is called at every door. |
| 5.2 | `PersistentLayer` + `ViewportPlaceholder`, behind a flag, with the current shell still driving. Proves the follow-loop before anything moves. | ☐ | | Accept: Stage/3D/timeline track their placeholder through splitter drags and context switches with zero React state per frame; Art-Net canary green. |
| 5.3 | Render the tree in `WorkspaceShell` behind `layout.docking`, defaulting **off**. Both paths ship until 5.6. | ☐ | | Accept: with the flag on, every context renders identically to the flag off — screenshot/DOM parity per context. |
