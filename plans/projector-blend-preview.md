...
- **Headless mode** *(written when it was a separate `headless.html`/`headless.tsx`/`HeadlessRunner.tsx` entry — that fork was **retired in P6**; headless is now `index.html?headless=1`, `main/index.ts` + `App.tsx:145-149`. The conclusion is unchanged, the mechanism is one entry with a gate):* it *does* render EFFECT via `Stage`, but no transport broadcast ever reaches it (no projector ports), so `sharedEpoch` stays its own `timeOrigin` → `effectTimeSec() ≡ performance.now()/1000` → byte-identical output. Unaffected **via the identity default**, not "no EFFECT."
